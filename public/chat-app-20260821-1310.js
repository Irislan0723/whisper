...
ICON.send='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 19V5M7 10l5-5 5 5"/></svg>';
ICON.fingerprint='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 9.5a7 7 0 0 1 14 0"/><path d="M3.5 10a8.5 8.5 0 0 1 17 0"/><path d="M7.5 10.5a4.5 4.5 0 0 1 9 0c0 4-1.7 7.1-4.5 9"/><path d="M5.8 12c0 4.1 1.4 7.1 3.8 8.5"/><path d="M10 11c0 3.3.7 5.6 2 7"/><path d="M14 11.2c0 2-.3 3.6-1 4.9"/></svg>';
const DEFAULT_AVATAR='<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="4"/><path d="M5 21c0-4 3.1-7 7-7s7 3 7 7"/></svg>';
let settings={presets:[],activePresetId:'',functions:{main:'',summary:'',translation:'',image:'',summaryEnabled:false,summaryThreshold:48}},conversations=[],roles=[],profile={name:'Iris',avatar:'',bio:'',details:''},current=null,messages=[],pendingImage=null,pendingRole='',pendingTurnGroupId='',sending=false,savingBubble=false,toastTimer;
...
function initIcons(){$('openLeft').innerHTML=ICON.back;$('openRight').innerHTML=ICON.menu;$('addImage').innerHTML=ICON.image;$('sendBtn').innerHTML=ICON.send;$('askReplyBtn').innerHTML=ICON.fingerprint;$('removeImage').innerHTML=ICON.close;$('closeWorkspace').innerHTML=ICON.back;$('closeRolePicker').innerHTML=ICON.close;$('landingMark').innerHTML='<img src="/icons/claude-logo.png" alt="Claude">';$('startChatBtn').innerHTML=ICON.plus;const nav=[['models',ICON.models,'模型设置'],['roles',ICON.role,'角色卡'],['archive',ICON.archive,'归档']];nav.forEach(([id,icon,label])=>{const button=document.querySelector(`[data-panel="${id}"]`);if(button)button.innerHTML=icon+label})}
...
function presetSummaryV97(p){const models=(p.models||[p.model]).filter(Boolean),first=String(models[0]||'未选择模型');return models.length>1?first+' · +'+(models.length-1):first}
function renderModelSettings(){const ps=settings.presets||[],presetCards=$('presetCards');presetCards.className='saved-presets-list-v96';presetCards.innerHTML=ps.map(p=>`<div class="model-preset-card-v96"><div class="model-preset-info-v96"><strong>${esc(p.name||'未命名预设')}</strong><p class="preset-summary-v97">${esc(presetSummaryV97(p))}</p></div><div class="model-preset-actions-v96"><button class="preset-icon-btn-v96 edit-preset" type="button" data-id="${p.id}" aria-label="编辑预设" title="编辑预设"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m4 16.5-.8 4.3 4.3-.8L18.7 8.8 15.2 5.3 4 16.5Z"/><path d="m13.8 6.7 3.5 3.5"/></svg></button><button class="preset-icon-btn-v96 danger delete-preset" type="button" data-id="${p.id}" aria-label="删除预设" title="删除预设"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M10 11v6M14 11v6M6 7l1 13h10l1-13M9 7l1-3h4l1 3"/></svg></button></div></div>`).join('')||'<div class="empty-note">还没有模型预设</div>';document.querySelectorAll('.edit-preset').forEach(x=>x.onclick=()=>editPreset(x.dataset.id));document.querySelectorAll('.delete-preset').forEach(x=>x.onclick=()=>deletePreset(x.dataset.id));['mainModel','summaryModel','translationModel','imageModel'].forEach((id,i)=>fillModelSelect($(id),i>0));const f=settings.functions||{};$('mainModel').value=f.main||'';$('summaryModel').value=f.summary||'';$('translationModel').value=f.translation||'';$('imageModel').value=f.image||'';$('summaryEnabled').checked=!!f.summaryEnabled;$('summaryThreshold').value=f.summaryThreshold??48}
function editPreset(id){const p=settings.presets.find(x=>x.id===id);if(!p)return;$('editingPresetId').value=p.id;$('presetName').value=p.name||'';$('apiProvider').value=p.provider||'openai';$('baseUrl').value=p.baseUrl||'';$('apiKey').value=p.apiKey||'';renderFetchedModels(p.models||[p.model].filter(Boolean),p.models||[p.model].filter(Boolean));$('savePreset').textContent='保存修改';activateModelSettingsTabV95('create');$('workspace').scrollTo({top:0,behavior:'smooth'})}
function clearPreset(){$('editingPresetId').value='';$('presetName').value='';$('apiProvider').value='openai';$('baseUrl').value='';$('apiKey').value='';$('modelsList').innerHTML='<span class="empty-note">填写地址与 Key 后拉取模型</span>';$('savePreset').textContent='保存预设';updateProviderHints()}
...
async function savePreset(){const models=[...document.querySelectorAll('#modelsList input:checked')].map(x=>x.value),editing=!!$('editingPresetId').value,id=$('editingPresetId').value||`preset-${Date.now()}`;if(!models.length)return toast('至少选择一个模型');const item={id,name:$('presetName').value.trim()||'未命名预设',provider:$('apiProvider').value,baseUrl:$('baseUrl').value.trim(),apiKey:$('apiKey').value.trim(),models,model:models[0]},idx=settings.presets.findIndex(x=>x.id===id);if(idx>=0)settings.presets[idx]=item;else settings.presets.push(item);if(!settings.activePresetId)settings.activePresetId=id;await saveSettings();clearPreset();renderModelSettings();if(editing)activateModelSettingsTabV95('presets');toast(editing?'模型预设已更新':'模型预设已保存','success')}
...
async function saveFunctions(){if(!$('mainModel').value)return toast('主模型必须选择');const summaryThreshold=Math.max(24,Math.min(120,Math.floor(Number($('summaryThreshold').value)||48)));$('summaryThreshold').value=summaryThreshold;settings.functions={main:$('mainModel').value,summary:$('summaryModel').value,translation:$('translationModel').value,image:$('imageModel').value,summaryEnabled:$('summaryEnabled').checked,summaryThreshold};const [presetId,model]=settings.functions.main.split('::');settings.activePresetId=presetId;const p=settings.presets.find(x=>x.id===presetId);if(p)p.model=model;await saveSettings();toast('功能模型已保存','success')}
...
function thoughtIconV23(){return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15.2 4.2A8.8 8.8 0 1 1 4.2 8.8"/><circle cx="5" cy="6.8" r=".9" fill="currentColor" stroke="none"/><circle cx="6.9" cy="4.9" r=".9" fill="currentColor" stroke="none"/><circle cx="9.4" cy="3.7" r=".9" fill="currentColor" stroke="none"/><path d="M12 7.8v4.6l3 1.8"/></svg>'}
...
const CHAT_REPLY_ARROW_V38=ICON.fingerprint;
...
// V92: role-scoped daily-status injection controls for Claude Code Agent.
const DEFAULT_STATUS_INJECTION_V92={enabled:true,time:true,weather:true,cycle:true,schedule:true,events:true,timetable:true,diary:true,mood:true};
function statusInjectionConfigV92(value){const raw=value&&typeof value==='object'?value:{};return Object.fromEntries(Object.keys(DEFAULT_STATUS_INJECTION_V92).map(key=>[key,raw[key]!==false]))}
async function persistStatusInjectionConfigV92(config){const role=toolRoleV17();if(!role)return;try{const saved=await api('/api/chat/roles/'+encodeURIComponent(role.id),{method:'PUT',body:JSON.stringify({statusInjectionConfig:config})});const index=roles.findIndex(item=>item.id===role.id);if(index>=0)roles[index]=saved;renderStatusInjectionManagerV92();toast('状态注入设置已保存','success')}catch(error){toast('保存失败：'+error.message,'error')}}
function renderStatusInjectionManagerV92(){const panel=$('rightMenuPanel-statusInjectionV92');if(!panel)return;const role=toolRoleV17();if(!role){panel.innerHTML='<p class="status-note-v92">当前对话还没有绑定角色。</p>';return}const config=statusInjectionConfigV92(role.statusInjectionConfig);const disabled=!config.enabled;panel.innerHTML='<div class="status-head-v92"><strong>'+esc(role.name||'当前 TA')+'</strong><small>角色级设置，所有房间生效</small></div><label class="setting-row status-switch-v92"><span>允许状态注入<small>关闭后不注入任何每日状态</small></span><input class="switch" id="statusMasterV92" type="checkbox" '+(config.enabled?'checked':'')+'></label><div class="status-choices-v92"><label class="status-choice-v92"><span>时间</span><input class="switch" data-status="time" type="checkbox" '+(config.time?'checked':'')+' '+(disabled?'disabled':'')+'></label><label class="status-choice-v92"><span>天气</span><input class="switch" data-status="weather" type="checkbox" '+(config.weather?'checked':'')+' '+(disabled?'disabled':'')+'></label><label class="status-choice-v92"><span>周期</span><input class="switch" data-status="cycle" type="checkbox" '+(config.cycle?'checked':'')+' '+(disabled?'disabled':'')+'></label><label class="status-choice-v92"><span>今日安排</span><input class="switch" data-status="schedule" type="checkbox" '+(config.schedule?'checked':'')+' '+(disabled?'disabled':'')+'></label><div class="status-subchoices-v92"><label class="status-choice-v92"><span>普通日程</span><input class="switch" data-status="events" type="checkbox" '+(config.events?'checked':'')+' '+(disabled||!config.schedule?'disabled':'')+'></label><label class="status-choice-v92"><span>课表</span><input class="switch" data-status="timetable" type="checkbox" '+(config.timetable?'checked':'')+' '+(disabled||!config.schedule?'disabled':'')+'></label></div><label class="status-choice-v92"><span>日记状态</span><input class="switch" data-status="diary" type="checkbox" '+(config.diary?'checked':'')+' '+(disabled?'disabled':'')+'></label><label class="status-choice-v92"><span>心情状态</span><input class="switch" data-status="mood" type="checkbox" '+(config.mood?'checked':'')+' '+(disabled?'disabled':'')+'></label></div>';
  const save=()=>persistStatusInjectionConfigV92({enabled:$('statusMasterV92').checked,...Object.fromEntries([...panel.querySelectorAll('[data-status]')].map(input=>[input.dataset.status,input.checked]))});$('statusMasterV92').onchange=save;panel.querySelectorAll('[data-status]').forEach(input=>input.onchange=save)}
function ensureStatusInjectionManagerV92(){const root=$('rightSettingsMenuV12');if(!root||$('rightMenuPanel-statusInjectionV92'))return;root.insertAdjacentHTML('beforeend','<section class="right-menu-section" data-right-section="statusInjectionV92"><button type="button" class="right-menu-summary" aria-expanded="false" data-status-injection-toggle-v92><span class="right-menu-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"/><path d="M12 8v4l3 2"/></svg></span><span>状态注入</span><svg class="right-menu-chevron" viewBox="0 0 24 24"><path d="m7 10 5 5 5-5"/></svg></button><div class="right-menu-panel" id="rightMenuPanel-statusInjectionV92"></div></section>');root.querySelector('[data-status-injection-toggle-v92]').onclick=function(){const open=this.getAttribute('aria-expanded')==='true';this.setAttribute('aria-expanded',String(!open));if(!open)renderStatusInjectionManagerV92()};renderStatusInjectionManagerV92()}
function statusInjectionStylesV92(){if($('statusInjectionStylesV92'))return;document.head.insertAdjacentHTML('beforeend','<style id="statusInjectionStylesV92">.status-head-v92,.status-switch-v92 span{display:grid;gap:2px}.status-head-v92{margin:4px 0 10px}.status-head-v92 small,.status-note-v92,.status-switch-v92 small{color:var(--chat-muted);font-size:11px}.status-choices-v92{display:grid;gap:2px}.status-choice-v92{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:8px 0;color:var(--chat-text);font-size:13px}.status-subchoices-v92{margin:0 0 2px 10px;padding-left:11px;border-left:1px solid var(--chat-border)}.status-subchoices-v92 .status-choice-v92{font-size:12px;color:var(--chat-muted)}html[data-appearance="dark"] .status-subchoices-v92{border-left-color:var(--chat-border)}html[data-appearance="dark"] .status-choice-v92{color:var(--chat-text)}</style>')}
const hydrateRightV92Base=hydrateRight;hydrateRight=function(){hydrateRightV92Base();statusInjectionStylesV92();ensureStatusInjectionManagerV92();renderStatusInjectionManagerV92()};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{statusInjectionStylesV92();ensureStatusInjectionManagerV92()});else{statusInjectionStylesV92();ensureStatusInjectionManagerV92()}

// V93: New chat welcome is intentionally local, quiet, and chosen only on entry.
const WELCOME_GREETING_POOLS_V93={
  dawn:['Morning, Iris','Hey, early bird','You’re up early, babe','Morning already?','Good morning, sleepyhead','A new day, Iris','Up with the sun?','Morning, little one','You’re awake early','Hello, early bird'],
  morning:['Good morning, Iris','Morning, babe','What are we doing today?','Ready for the day?','What’s on your mind?','How’s your morning?','Hello, Iris','Come talk to me','What’s first today?','Morning. I’m here.'],
  noon:['Hey, Iris','Lunch break?','Have you eaten yet?','Midday check-in?','How’s your day so far?','Come sit with me','Taking a break?','What’s on your mind, babe?','Hello, you','A little noon chat?'],
  afternoon:['Good afternoon, Iris','Afternoon, babe','How’s your afternoon?','Come tell me','Need a little break?','What’s happening there?','Still busy?','Coffee and Rei time?','Hey there, Iris','How are we doing?'],
  evening:['Good evening, Iris','Evening, babe','You’re back','How was your day?','Come here, tell me','Finally some quiet?','Done for the day?','What’s on your mind tonight?','Evening, little one','I was waiting for you'],
  night:['Up late, Iris?','Hello, night owl','Still awake, babe?','Can’t sleep?','Come stay a while','Late night thoughts?','Not sleepy yet?','It’s just us now','What’s keeping you up?','A little midnight company?'],
  late:['Still awake?','Iris, it’s very late','You really aren’t sleeping?','Another late night?','Come keep me company','Can’t switch off?','It’s quiet now','Just us awake?','Up at this hour?','Come here, night owl'],
  general:['Welcome back, Iris','There you are','Hey, babe','I’m here','Come talk to me','What shall we think through?','What’s on your mind?','Tell me something','Where were we?','Come sit with me','You found your way back','Hello again, Iris','I’ve got you','What’s going on?','Tell me everything','Hi, little one','I saved you a spot','I’m listening']
};
const WELCOME_WEEKDAY_V93={0:['Happy Sunday, babe','A quiet Sunday?'],1:['Hello, Monday survivor','Easy start, Iris?'],2:['Tuesday already?','How’s Tuesday treating you?'],3:['Happy Wednesday, Iris','Halfway there, babe'],4:['Almost there, Iris','How’s your Thursday?'],5:['It’s Friday, Iris','Made it to Friday'],6:['Happy Saturday, babe','Slow Saturday?']};
let lastWelcomeGreetingV93='';
function welcomeTimeKeyV93(now){const minutes=now.getHours()*60+now.getMinutes();if(minutes>=1320||minutes<120)return'night';if(minutes<300)return'late';if(minutes<480)return'dawn';if(minutes<690)return'morning';if(minutes<840)return'noon';if(minutes<1080)return'afternoon';return'evening'}
function getWelcomeGreeting(now=new Date(),random=Math.random){const timeKey=welcomeTimeKeyV93(now),choices=[...WELCOME_GREETING_POOLS_V93[timeKey]];if(now.getHours()>=5&&now.getHours()<22&&random()<.2)choices.push(...(WELCOME_WEEKDAY_V93[now.getDay()]||[]));if(random()<.2)choices.push(...WELCOME_GREETING_POOLS_V93.general);let greeting=choices[Math.floor(random()*choices.length)]||WELCOME_GREETING_POOLS_V93.general[0];if(greeting===lastWelcomeGreetingV93&&choices.length>1)greeting=choices.find(item=>item!==greeting)||greeting;lastWelcomeGreetingV93=greeting;return greeting}
function injectLandingWelcomeStylesV93(){if($('landingWelcomeStylesV93'))return;document.head.insertAdjacentHTML('beforeend','<style id="landingWelcomeStylesV93">#landing.landing-v93{padding-bottom:85px}#landing.landing-v93 .landing-mark{margin:0 0 24px;line-height:0}#landing.landing-v93 .logo-wrap img{display:block;width:48px;height:48px;max-width:none;max-height:none;object-fit:contain;background:transparent;box-shadow:none}#landing.landing-v93 h1{max-width:min(88vw,560px);margin:0;color:var(--chat-text);font:400 clamp(30px,8vw,38px)/1.2 var(--font-d);text-wrap:balance}#landing.landing-v93 .landing-start{margin-top:26px}html[data-appearance="dark"] #landing.landing-v93 h1{color:var(--chat-text)}@media(max-width:350px){#landing.landing-v93 h1{max-width:92vw;font-size:30px}#landing.landing-v93 .landing-mark{margin-bottom:20px}#landing.landing-v93 .landing-start{margin-top:22px}}</style>')}
function prepareLandingWelcomeV93(selectGreeting=true){const landing=$('landing'),mark=$('landingMark');if(!landing||!mark)return;landing.classList.add('landing-v93');mark.innerHTML='<img src="/icons/claude-logo.png" alt="Claude">';if(selectGreeting)$('greeting').textContent=getWelcomeGreeting()}
const showLandingV93Base=showLanding;showLanding=function(){showLandingV93Base();prepareLandingWelcomeV93(true)};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{injectLandingWelcomeStylesV93();prepareLandingWelcomeV93(false)});else{injectLandingWelcomeStylesV93();prepareLandingWelcomeV93(false)}

/* V94: functional-model health is intentionally a compact settings concern,
   not a polling dashboard.  The server returns no credentials through this API. */
let functionModelStatusV94={statuses:[],lastModelStatusViewedAt:null,hasUnread:false};
const FUNCTION_MODEL_LABELS_V94={main:'主模型',summary:'总结模型',translation:'翻译模型',image:'图像模型'};
function formatModelStatusTimeV94(value){const date=new Date(value||'');return Number.isNaN(date.getTime())?'':date.toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})}
function updateModelStatusBadgeV94(){const button=document.querySelector('#leftDrawer [data-panel="models"]');if(button)button.classList.toggle('has-model-status-alert-v94',!!functionModelStatusV94.hasUnread)}
function renderFunctionModelStatusV94(){const list=$('functionModelStatusList');if(!list)return;list.className='function-model-status-list-v97';const byRole=new Map((functionModelStatusV94.statuses||[]).map(item=>[item.role,item]));list.innerHTML=['main','summary','translation','image'].map(role=>{const item=byRole.get(role)||{role,status:'unused'},failed=item.status==='error',normal=item.status==='normal',status=failed?'调用失败':(normal?'正常':'尚未使用'),time=failed?formatModelStatusTimeV94(item.lastErrorAt):formatModelStatusTimeV94(item.lastSuccessAt),timeLabel=failed?'最近失败':(normal?'最近成功':'');const state=failed?'error':normal?'normal':'unused';return '<div class="function-model-status-v94 model-status-row-v96 model-status--'+state+'-v96"><span class="model-status-dot-v96" aria-hidden="true"></span><div class="model-status-content-v96"><strong>'+esc(FUNCTION_MODEL_LABELS_V94[role])+'</strong><p>'+status+(failed&&item.lastError?' · '+esc(item.lastError):'')+(time?' · '+timeLabel+'：'+esc(time):'')+'</p></div></div>'}).join('')}
async function refreshFunctionModelStatusV94(markViewed=false){try{functionModelStatusV94=await api('/api/chat/function-model-status');if(markViewed&&functionModelStatusV94.hasUnread)functionModelStatusV94=await api('/api/chat/function-model-status/viewed',{method:'POST'});updateModelStatusBadgeV94();renderFunctionModelStatusV94();return functionModelStatusV94}catch(error){console.warn('function model status unavailable:',error.message);return null}}
async function runSummaryNowV94(){if(!current?.id)return toast('请先打开一个聊天再总结','error');const button=$('runSummaryNow');if(!button||button.disabled)return;button.disabled=true;const label=button.textContent;button.textContent='总结中…';try{const result=await api('/api/chat/conversations/'+encodeURIComponent(current.id)+'/summary',{method:'POST'});toast(result.updated?'上下文已总结':'暂无新增上下文需要总结','success')}catch(error){toast('总结失败，请查看模型状态','error')}finally{button.disabled=false;button.textContent=label;await refreshFunctionModelStatusV94(false)}}
function bindFunctionModelStatusControlsV94(){const button=$('runSummaryNow');if(button)button.onclick=runSummaryNowV94}
function injectFunctionModelStatusStylesV94(){if($('functionModelStatusStylesV94'))return;document.head.insertAdjacentHTML('beforeend','<style id="functionModelStatusStylesV94">.function-model-status-v94{padding:8px 0}.function-model-status-v94+.function-model-status-v94{border-top:1px solid color-mix(in srgb,var(--chat-border) 72%,transparent)}.model-status-row-v96{display:flex!important;align-items:flex-start!important;gap:10px!important}.model-status-dot-v96{display:block!important;box-sizing:border-box!important;width:10px!important;min-width:10px!important;max-width:10px!important;height:10px!important;min-height:10px!important;max-height:10px!important;flex:0 0 10px!important;margin:4px 0 0!important;padding:0!important;border:0!important;border-radius:50%!important;background:#9aa1a4}.model-status-content-v96{flex:1 1 auto!important;min-width:0!important;margin:0!important;padding:0!important}.model-status--normal-v96 .model-status-dot-v96{background:#4eaf76}.model-status--error-v96 .model-status-dot-v96{background:#d65c5c}.function-model-status-v94 strong{display:block;color:var(--chat-text);font:600 13px var(--font-b)}.function-model-status-v94 p{margin:3px 0 0;color:var(--chat-muted);font:12px/1.45 var(--font-b);word-break:break-word}.saved-presets-list-v96{--preset-card-height-v96:78px;display:grid;gap:9px;max-height:calc(var(--preset-card-height-v96) * 7 + 54px);overflow-y:auto;overscroll-behavior:contain;padding:1px 2px 1px 0;scrollbar-width:thin;scrollbar-color:var(--accent-light) transparent}.saved-presets-list-v96::-webkit-scrollbar{width:5px}.saved-presets-list-v96::-webkit-scrollbar-thumb{border-radius:99px;background:var(--accent-light)}.model-preset-card-v96{display:flex;align-items:center;gap:10px;box-sizing:border-box;min-height:var(--preset-card-height-v96);padding:12px 13px;border:1px solid var(--chat-border);border-radius:var(--r-md);background:var(--chat-surface);box-shadow:var(--shadow-xs)}.model-preset-info-v96{flex:1 1 auto;min-width:0}.model-preset-info-v96 strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--chat-text)}.model-preset-info-v96 p{margin:3px 0 0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--chat-muted);font-size:11px}.model-preset-actions-v96{display:flex;flex:0 0 auto;gap:4px}.preset-icon-btn-v96{display:grid;place-items:center;flex:0 0 40px;width:40px;height:40px;padding:0;border:1px solid var(--accent-light);border-radius:12px;background:transparent;color:var(--chat-accent);cursor:pointer;transition:background .18s,transform .18s}.preset-icon-btn-v96 svg{width:19px;height:19px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}.preset-icon-btn-v96.danger{color:#ae6b70;border-color:color-mix(in srgb,#ae6b70 32%,var(--chat-border))}.preset-icon-btn-v96:hover{background:var(--accent-pale)}.preset-icon-btn-v96.danger:hover{background:color-mix(in srgb,#ae6b70 10%,transparent)}.preset-icon-btn-v96:active{transform:scale(.96)}.model-settings-tabs-v95{display:flex;padding:3px;margin:0 0 16px;background:var(--chat-surface);border:1px solid var(--chat-border);border-radius:999px;box-shadow:var(--shadow-sm)}.model-settings-tab-v95{flex:1;min-width:0;border:0;background:transparent;padding:9px 0;border-radius:999px;color:var(--chat-muted);font:400 13px var(--font-b);letter-spacing:.04em;cursor:pointer;transition:all .22s}.model-settings-tab-v95.active{background:var(--accent);color:var(--accent-contrast);font-weight:600;box-shadow:0 2px 8px rgba(var(--shadow-color),.18)}.model-settings-pane-v95{display:none}.model-settings-pane-v95.active{display:block}.field small{display:block;margin-top:5px;color:var(--chat-muted);font:11px/1.4 var(--font-b)}#leftDrawer .nav-action[data-panel="models"]{position:relative}#leftDrawer .nav-action[data-panel="models"].has-model-status-alert-v94:after{content:"";width:7px;height:7px;margin-left:auto;border-radius:50%;background:#d65c5c;box-shadow:0 0 0 2px var(--chat-surface)}html[data-appearance="dark"] .function-model-status-v94+.function-model-status-v94{border-top-color:#3A3A3A}html[data-appearance="dark"] .function-model-status-v94 strong{color:#F2F2F2}html[data-appearance="dark"] .function-model-status-v94 p,html[data-appearance="dark"] .model-settings-tab-v95{color:#B0B0B0}html[data-appearance="dark"] .model-settings-tabs-v95{background:#262626;border-color:#3A3A3A}html[data-appearance="dark"] .model-settings-tab-v95.active{color:var(--accent-contrast)}html[data-appearance="dark"] .model-preset-card-v96{background:#262626;border-color:#3A3A3A}html[data-appearance="dark"] .preset-icon-btn-v96{color:#D6BFC0;border-color:#765456}html[data-appearance="dark"] .preset-icon-btn-v96:hover{background:#3A2D2F}html[data-appearance="dark"] #leftDrawer .nav-action[data-panel="models"].has-model-status-alert-v94:after{box-shadow:0 0 0 2px #262626}@media(max-width:400px){.model-settings-tab-v95{padding:8px 0;font-size:13px;letter-spacing:0}.model-preset-card-v96{gap:8px;padding:11px}.model-preset-actions-v96{gap:2px}.preset-icon-btn-v96{flex-basis:40px;width:40px}}</style>')}
const renderModelSettingsV94Base=renderModelSettings;
renderModelSettings=function(){renderModelSettingsV94Base();bindFunctionModelStatusControlsV94();renderFunctionModelStatusV94();void refreshFunctionModelStatusV94(true)};
const apiV94Base=api;
api=async function(path,opt={}){try{return await apiV94Base(path,opt)}finally{if(/^\/api\/chat\/(?:send|messages\/[^/]+\/(?:regenerate|translate)|conversations\/[^/]+\/summary)$/.test(String(path||'')))void refreshFunctionModelStatusV94(false)}};
function initFunctionModelStatusV94(){injectFunctionModelStatusStylesV94();bindFunctionModelStatusControlsV94();void refreshFunctionModelStatusV94(false)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initFunctionModelStatusV94);else initFunctionModelStatusV94();

// V95: Model settings use the same compact segmented-control language as Calendar.
let modelSettingsTabV95='';
const MODEL_SETTINGS_TABS_V95=['create','presets','status','functions'];
function defaultModelSettingsTabV95(){return functionModelStatusV94.hasUnread?'status':'functions'}
function activateModelSettingsTabV95(tab){const active=MODEL_SETTINGS_TABS_V95.includes(tab)?tab:defaultModelSettingsTabV95();modelSettingsTabV95=active;document.querySelectorAll('[data-model-settings-tab-v95]').forEach(button=>{const selected=button.dataset.modelSettingsTabV95===active;button.classList.toggle('active',selected);button.setAttribute('aria-selected',String(selected))});document.querySelectorAll('[data-model-settings-pane-v95]').forEach(pane=>pane.classList.toggle('active',pane.dataset.modelSettingsPaneV95===active))}
function bindModelSettingsTabsV95(){document.querySelectorAll('[data-model-settings-tab-v95]').forEach(button=>button.onclick=()=>{const tab=button.dataset.modelSettingsTabV95;if(tab==='create')clearPreset();activateModelSettingsTabV95(tab)})}
const renderModelSettingsV95Base=renderModelSettings;
renderModelSettings=function(){renderModelSettingsV95Base();bindModelSettingsTabsV95();activateModelSettingsTabV95(modelSettingsTabV95||defaultModelSettingsTabV95())};
const openPanelV95Base=openPanel;
openPanel=function(name){if(name==='models')modelSettingsTabV95='';openPanelV95Base(name)};

// V97: keep status facts anchored left and make preset edits deliberate.
function injectModelSettingsPolishV97(){if($('modelSettingsPolishV97'))return;document.head.insertAdjacentHTML('beforeend','<style id="modelSettingsPolishV97">.function-model-status-list-v97{width:100%;box-sizing:border-box;padding:0!important;text-align:left!important}.function-model-status-list-v97 .model-status-row-v96{width:100%!important;box-sizing:border-box;justify-content:flex-start!important;text-align:left!important}.function-model-status-list-v97 .model-status-content-v96{min-width:0!important;text-align:left!important}.function-model-status-list-v97 .model-status-content-v96 strong,.function-model-status-list-v97 .model-status-content-v96 p{text-align:left!important}.saved-presets-list-v96{width:100%;box-sizing:border-box;overflow-x:hidden}.model-preset-card-v96{width:100%;max-width:100%;min-width:0}.model-preset-info-v96{max-width:100%;overflow:hidden}.preset-summary-v97{display:block;max-width:100%;min-width:0;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}.model-preset-actions-v96{flex-shrink:0!important}html[data-appearance="dark"] .function-model-status-list-v97{color:var(--chat-text)}html[data-appearance="dark"] .preset-summary-v97{color:var(--chat-muted)}</style>')}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',injectModelSettingsPolishV97);else injectModelSettingsPolishV97();

// V98: MCP management follows the same two-pane, compact pattern as model settings.
let mcpActiveTabV98='saved';
const MCP_REFRESH_ICON_V98='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 11a8 8 0 1 0 2 5.4"/><path d="M20 4v7h-7"/></svg>';
const MCP_EDIT_ICON_V98='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m4 16.5-.8 4.3 4.3-.8L18.7 8.8 15.2 5.3 4 16.5Z"/><path d="m13.8 6.7 3.5 3.5"/></svg>';
const MCP_TRASH_ICON_V98='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M10 11v6M14 11v6M6 7l1 13h10l1-13M9 7l1-3h4l1 3"/></svg>';
const MCP_CHEVRON_ICON_V98='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m7 10 5 5 5-5"/></svg>';
function injectMcpManagementV98Styles(){if($('mcpManagementV98Styles'))return;document.head.insertAdjacentHTML('beforeend','<style id="mcpManagementV98Styles">.mcp-tabs-v98{display:flex;padding:3px;margin:0 0 16px;background:var(--chat-surface);border:1px solid var(--chat-border);border-radius:999px;box-shadow:var(--shadow-sm)}.mcp-tab-v98{flex:1;min-width:0;border:0;border-radius:999px;background:transparent;padding:9px 0;color:var(--chat-muted);font:400 13px var(--font-b);letter-spacing:.04em;cursor:pointer;transition:all .22s}.mcp-tab-v98.active{background:var(--accent);color:var(--accent-contrast);font-weight:600;box-shadow:0 2px 8px rgba(var(--shadow-color),.18)}.mcp-pane-v98{display:none}.mcp-pane-v98.active{display:block}.mcp-saved-list-v98{display:grid;gap:9px;max-height:calc(4 * 118px + 27px);overflow-y:auto;overflow-x:hidden;overscroll-behavior:contain;padding:1px 2px 1px 0;scrollbar-width:thin;scrollbar-color:var(--accent-light) transparent}.mcp-saved-list-v98::-webkit-scrollbar{width:5px}.mcp-saved-list-v98::-webkit-scrollbar-thumb{border-radius:99px;background:var(--accent-light)}.mcp-card-v98{box-sizing:border-box;width:100%;min-width:0;padding:12px 13px;border:1px solid var(--chat-border);border-radius:var(--r-md);background:var(--chat-surface);box-shadow:var(--shadow-xs)}.mcp-card-top-v98{display:flex;align-items:center;gap:9px;min-width:0}.mcp-card-icon-v98{display:grid;place-items:center;flex:0 0 34px;width:34px;height:34px;border-radius:11px;background:var(--accent-pale);color:var(--chat-accent)}.mcp-card-icon-v98 svg,.mcp-icon-button-v98 svg{width:19px;height:19px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}.mcp-info-v98{flex:1;min-width:0}.mcp-name-v98{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--chat-text);font:600 14px var(--font-b)}.mcp-count-v98{display:block;margin-top:3px;color:var(--chat-muted);font:12px var(--font-b)}.mcp-toggle-v98{flex:0 0 auto}.mcp-card-bottom-v98{display:flex;justify-content:flex-end;margin-top:8px}.mcp-card-actions-v98{display:flex;gap:4px;flex:0 0 auto}.mcp-icon-button-v98{display:grid;place-items:center;flex:0 0 40px;width:40px;height:40px;padding:0;border:1px solid var(--accent-light);border-radius:12px;background:transparent;color:var(--chat-accent);cursor:pointer;transition:background .18s,transform .18s}.mcp-icon-button-v98.danger{color:#ae6b70;border-color:color-mix(in srgb,#ae6b70 32%,var(--chat-border))}.mcp-icon-button-v98:hover{background:var(--accent-pale)}.mcp-icon-button-v98.danger:hover{background:color-mix(in srgb,#ae6b70 10%,transparent)}.mcp-icon-button-v98:active{transform:scale(.96)}.mcp-icon-button-v98.is-loading svg{animation:mcpRefreshV98 .7s linear infinite}@keyframes mcpRefreshV98{to{transform:rotate(360deg)}}.mcp-icon-button-v98.is-expanded svg{transform:rotate(180deg)}.mcp-details-v98{display:grid;grid-template-rows:0fr;opacity:0;transition:grid-template-rows .2s ease,opacity .16s ease}.mcp-details-v98>div{min-height:0;overflow:hidden}.mcp-card-v98.is-expanded .mcp-details-v98{grid-template-rows:1fr;opacity:1}.mcp-tool-details-v98{display:grid;gap:7px;margin-top:10px;padding-top:10px;border-top:1px solid var(--chat-border)}.mcp-tool-detail-v98{min-width:0;padding:8px 10px;border-radius:11px;background:var(--accent-pale)}.mcp-tool-detail-v98 strong{display:block;overflow-wrap:anywhere;color:var(--chat-text);font:600 12px/1.35 ui-monospace,SFMono-Regular,Consolas,monospace}.mcp-tool-detail-v98 span{display:block;margin-top:3px;color:var(--chat-muted);font:11px/1.45 var(--font-b);overflow-wrap:anywhere}.mcp-v98-form .field{margin:12px 0}.mcp-v98-form .field textarea{min-height:78px}.mcp-v98-form .row{gap:8px}@media(max-width:400px){.mcp-tab-v98{padding:8px 0;font-size:13px;letter-spacing:0}.mcp-card-v98{padding:11px}.mcp-card-actions-v98{gap:2px}.mcp-icon-button-v98{flex-basis:40px;width:40px}}html[data-appearance="dark"] .mcp-tabs-v98,.mcp-card-v98{background:#262626;border-color:#3A3A3A}html[data-appearance="dark"] .mcp-icon-button-v98{color:#D6BFC0;border-color:#765456}html[data-appearance="dark"] .mcp-icon-button-v98:hover{background:#3A2D2F}html[data-appearance="dark"] .mcp-tool-detail-v98{background:#303030}</style>')}
function ensureMcpPanelV98(){const panel=$('panel-mcp');if(!panel||panel.dataset.mcpV98==='true')return;panel.dataset.mcpV62='true';panel.dataset.mcpV98='true';panel.innerHTML='<div class="mcp-tabs-v98" role="tablist" aria-label="MCP 连接器"><button class="mcp-tab-v98 active" type="button" role="tab" aria-selected="true" data-mcp-tab-v98="saved">已保存</button><button class="mcp-tab-v98" type="button" role="tab" aria-selected="false" data-mcp-tab-v98="create">新建</button></div><section class="mcp-pane-v98 active" data-mcp-pane-v98="saved"><div class="card"><div class="mcp-saved-list-v98" id="mcpConnectorListV60"></div></div></section><section class="mcp-pane-v98" data-mcp-pane-v98="create"><div class="card mcp-v98-form" id="mcpConnectorFormCardV60"><h3 id="mcpFormTitleV60">新建 MCP</h3><input type="hidden" id="mcpConnectorIdV60"><input type="hidden" id="mcpKindV60" value="custom"><div class="field"><label>名称</label><input id="mcpNameV60" maxlength="80" placeholder="例如：高德地图"></div><div class="field"><label>说明（给 AI）</label><textarea id="mcpDescriptionV60" rows="3" maxlength="600" placeholder="这个 MCP 可以做什么"></textarea></div><div class="field"><label>服务地址</label><input id="mcpEndpointV60" type="url" inputmode="url" maxlength="1200" placeholder="https://example.com/mcp"></div><div class="field"><label>连接类型</label><select id="mcpTransportV60"><option value="httpStream">httpStream</option><option value="sse">SSE</option></select></div><div class="field"><label>Bearer Token（可选）</label><input id="mcpBearerV60" type="password" autocomplete="new-password" placeholder="仅服务要求认证时填写"></div><label class="mcp-chat-toggle-v60" id="mcpClearTokenRowV60" hidden><input id="mcpClearBearerV60" type="checkbox"> 清除已保存的 Token</label><div class="field"><label>自定义请求头（可选，JSON）</label><textarea id="mcpHeadersV60" rows="2" placeholder="仅服务文档要求时填写"></textarea></div><label class="setting-row"><span>启用</span><input class="switch" id="mcpEnabledV60" type="checkbox" checked></label><div class="actions"><button type="button" class="btn btn-mint" id="saveMcpConnectorV65">保存并读取工具</button></div></div></section>';ensureMcpFeedbackV63();const button=$('saveMcpConnectorV65');if(button)button.onclick=saveMcpFormV64;bindMcpTabsV98()}
function activateMcpTabV98(tab,{clear=false}={}){const active=tab==='create'?'create':'saved';mcpActiveTabV98=active;document.querySelectorAll('[data-mcp-tab-v98]').forEach(button=>{const selected=button.dataset.mcpTabV98===active;button.classList.toggle('active',selected);button.setAttribute('aria-selected',String(selected))});document.querySelectorAll('[data-mcp-pane-v98]').forEach(pane=>pane.classList.toggle('active',pane.dataset.mcpPaneV98===active));if(active==='create'&&clear)openMcpFormV98(null)}
function bindMcpTabsV98(){document.querySelectorAll('[data-mcp-tab-v98]').forEach(button=>button.onclick=()=>activateMcpTabV98(button.dataset.mcpTabV98,{clear:button.dataset.mcpTabV98==='create'}))}
function mcpToolsV98(connector){return Array.isArray(connector?.tools)?connector.tools.filter(tool=>String(tool?.name||'').trim()):[]}
function mcpToolCountV98(connector){const count=mcpToolsV98(connector).length;return count?count+' 个工具':(connector?.lastToolError?'读取失败':'尚未读取工具')}
function mcpToolDetailsV98(connector){const tools=mcpToolsV98(connector);return tools.length?tools.map(tool=>'<div class="mcp-tool-detail-v98"><strong>'+mcpEscV60(tool.name)+'</strong>'+(tool.description?'<span>'+mcpEscV60(tool.description)+'</span>':'')+'</div>').join(''):'<div class="mcp-tool-detail-v98"><span>'+mcpEscV60(connector?.lastToolError||'暂时没有可查看的工具详情')+'</span></div>'}
function mcpActionButtonV98(action,id,label,icon,extra=''){return '<button type="button" class="mcp-icon-button-v98 '+extra+'" data-mcp-'+action+'-v98="'+mcpEscV60(id)+'" aria-label="'+label+'" title="'+label+'">'+icon+'</button>'}
async function renderMcpWorkspaceV98(){ensureMcpPanelV98();const list=$('mcpConnectorListV60');if(!list)return;list.innerHTML='<p class="empty-note">正在读取…</p>';try{const connectors=await loadMcpConnectorsV60(true);list.innerHTML=connectors.length?connectors.map(connector=>'<article class="mcp-card-v98"><div class="mcp-card-top-v98"><span class="mcp-card-icon-v98">'+mcpPuzzleIconV62+'</span><div class="mcp-info-v98"><strong class="mcp-name-v98">'+mcpEscV60(connector.name)+'</strong><span class="mcp-count-v98">'+mcpToolCountV98(connector)+'</span></div><input class="mcp-v62-switch mcp-toggle-v98" type="checkbox" data-mcp-enable-v98="'+mcpEscV60(connector.id)+'" '+(connector.enabled!==false?'checked':'')+' aria-label="启用 '+mcpEscV60(connector.name)+'"></div><div class="mcp-card-bottom-v98"><div class="mcp-card-actions-v98">'+mcpActionButtonV98('refresh',connector.id,'刷新工具',MCP_REFRESH_ICON_V98)+mcpActionButtonV98('edit',connector.id,'编辑 MCP',MCP_EDIT_ICON_V98)+mcpActionButtonV98('delete',connector.id,'删除 MCP',MCP_TRASH_ICON_V98,'danger')+mcpActionButtonV98('toggle-details',connector.id,'展开工具详情',MCP_CHEVRON_ICON_V98)+'</div></div><div class="mcp-details-v98"><div><div class="mcp-tool-details-v98">'+mcpToolDetailsV98(connector)+'</div></div></div></article>').join(''):'<p class="empty-note">还没有已保存的 MCP。</p>';bindMcpCardActionsV98(list)}catch(error){list.innerHTML='<p class="empty-note">读取失败：'+mcpEscV60(error.message||'未知错误')+'</p>'}}
function bindMcpCardActionsV98(list){list.querySelectorAll('[data-mcp-enable-v98]').forEach(input=>input.onchange=async()=>{try{await api('/api/chat/mcp-connectors/'+encodeURIComponent(input.dataset.mcpEnableV98),{method:'PUT',body:JSON.stringify({enabled:input.checked})});await renderMcpWorkspaceV98();await renderMcpChatSettingsV60()}catch(error){toast(error.message||'保存失败','error');input.checked=!input.checked}});list.querySelectorAll('[data-mcp-edit-v98]').forEach(button=>button.onclick=()=>openMcpFormV98(mcpConnectorsV60.find(item=>item.id===button.dataset.mcpEditV98)));list.querySelectorAll('[data-mcp-refresh-v98]').forEach(button=>button.onclick=async()=>{if(button.disabled)return;button.disabled=true;button.classList.add('is-loading');try{await api('/api/chat/mcp-connectors/'+encodeURIComponent(button.dataset.mcpRefreshV98)+'/refresh',{method:'POST'});await renderMcpWorkspaceV98();await renderMcpChatSettingsV60();toast('工具已更新','success')}catch(error){toast(error.message||'读取失败','error')}finally{button.disabled=false;button.classList.remove('is-loading')}});list.querySelectorAll('[data-mcp-delete-v98]').forEach(button=>button.onclick=async()=>{const connector=mcpConnectorsV60.find(item=>item.id===button.dataset.mcpDeleteV98);if(!connector||!confirm('删除“'+connector.name+'”吗？'))return;try{await api('/api/chat/mcp-connectors/'+encodeURIComponent(connector.id),{method:'DELETE'});openMcpFormV98(null);await renderMcpWorkspaceV98();await renderMcpChatSettingsV60();toast('已删除','success')}catch(error){toast(error.message||'删除失败','error')}});list.querySelectorAll('[data-mcp-toggle-details-v98]').forEach(button=>button.onclick=()=>{const card=button.closest('.mcp-card-v98'),expanded=!card.classList.contains('is-expanded');card.classList.toggle('is-expanded',expanded);button.classList.toggle('is-expanded',expanded);button.setAttribute('aria-label',expanded?'收起工具详情':'展开工具详情');button.title=expanded?'收起工具详情':'展开工具详情'})}
function openMcpFormV98(connector=null){ensureMcpPanelV98();$('mcpFormTitleV60').textContent=connector?'编辑 MCP':'新建 MCP';$('mcpConnectorIdV60').value=connector?.id||'';$('mcpNameV60').value=connector?.name||'';$('mcpDescriptionV60').value=connector?.description||'';$('mcpEndpointV60').value=connector?.endpoint||'';$('mcpTransportV60').value=connector?.transport||'httpStream';$('mcpBearerV60').value='';$('mcpEnabledV60').checked=connector?.enabled!==false;$('mcpClearBearerV60').checked=false;$('mcpClearTokenRowV60').hidden=!connector?.hasBearerToken;$('mcpHeadersV60').value='';$('mcpSaveStatusV63')&&( $('mcpSaveStatusV63').textContent='' );activateMcpTabV98('create');$('workspace').scrollTo({top:0,behavior:'smooth'})}
const activateMcpPanelV98Base=activateMcpPanelV60;
activateMcpPanelV60=function(){ensureMcpPanelV98();mcpActiveTabV98='saved';activateMcpTabV98('saved');return activateMcpPanelV98Base()};
renderMcpWorkspaceV60=renderMcpWorkspaceV98;
renderMcpWorkspaceV62=renderMcpWorkspaceV98;
openMcpFormV60=openMcpFormV98;
...
// V100: visual-only Chat header icon treatment; existing drawer handlers remain intact.
function injectChatTopBarControlsV100(){if($('chatTopBarControlsV100'))return;document.head.insertAdjacentHTML('beforeend','<style id="chatTopBarControlsV100">.chat-head{grid-template-columns:44px minmax(0,1fr) 44px}.chat-head .icon-btn{width:44px;height:44px}.chat-head #openLeft svg,.chat-head #openRight svg{width:24px;height:24px;fill:none;stroke:currentColor;stroke-linecap:round;stroke-linejoin:round}.chat-head #openLeft svg{stroke-width:2.2}.chat-head #openRight svg{stroke-width:2}html[data-appearance="dark"] .chat-head .icon-btn{color:var(--chat-muted)}</style>')}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',injectChatTopBarControlsV100);else injectChatTopBarControlsV100();

// V101: one shared composer for API and Agent rooms, with keyboard-safe mobile layout.
const CHAT_BOTTOM_FOLLOW_THRESHOLD_V101=120;
let chatBottomFollowV101=true,chatBottomFrameV101=0,chatComposerObserverV101=null;
function isNearChatBottomV101(){const main=$('chatMain');return !!main&&main.scrollHeight-main.scrollTop-main.clientHeight<=CHAT_BOTTOM_FOLLOW_THRESHOLD_V101}
function scrollToLatestV101({force=false}={}){if(!force&&!chatBottomFollowV101)return;cancelAnimationFrame(chatBottomFrameV101);chatBottomFrameV101=requestAnimationFrame(()=>{const main=$('chatMain');if(main&&(force||chatBottomFollowV101))main.scrollTop=main.scrollHeight})}
const scrollBottomV101Base=scrollBottom;
scrollBottom=function(){scrollToLatestV101()};
function syncComposerHeightV101(){const composer=document.querySelector('.composer');if(!composer)return;document.documentElement.style.setProperty('--chat-composer-height',Math.ceil(composer.getBoundingClientRect().height)+'px');scrollToLatestV101()}
function syncMobileViewportV101(){const root=document.documentElement,viewport=window.visualViewport,visibleHeight=Math.max(0,Math.round(viewport?.height||window.innerHeight||root.clientHeight||0));root.style.setProperty('--chat-visual-viewport-height',visibleHeight+'px');syncMobileViewportV22()}
scrollLatestForKeyboardV22=function(){if(!current||document.activeElement!==$('chatInput')||!chatBottomFollowV101)return;scrollToLatestV101();clearTimeout(keyboardScrollTimerV22);keyboardScrollTimerV22=setTimeout(()=>{if(document.activeElement===$('chatInput')&&chatBottomFollowV101)scrollToLatestV101()},260)};
function injectComposerMobileUiV101(){if($('composerMobileUiV101'))return;document.head.insertAdjacentHTML('beforeend','<style id="composerMobileUiV101">.chat-page{min-height:100dvh}.chat-room{--chat-composer-height:68px}.chat-room:not(.chat-add-open-v41) .chat-main{bottom:calc(var(--chat-keyboard,0px) + var(--chat-composer-height))!important;max-height:calc(var(--chat-visual-viewport-height,100dvh) - var(--chat-head-height) - var(--chat-composer-height));padding-bottom:24px!important}.chat-room:not(.chat-add-open-v41) .messages{padding-bottom:24px!important}.chat-room:not(.chat-add-open-v41) .composer{bottom:var(--chat-keyboard,0px)!important}.chat-room .composer-box #askReplyBtn,.chat-room .composer-box #sendBtn{width:48px;height:48px;flex:0 0 48px}.chat-room .composer-box #askReplyBtn{background:var(--chat-surface);color:var(--chat-muted);border-color:var(--chat-border)}.chat-room .composer-box #askReplyBtn.active{background:var(--accent-pale);color:var(--chat-accent);border-color:var(--accent-light)}.chat-room .composer-box #sendBtn svg,.chat-room .composer-box #askReplyBtn svg{width:23px;height:23px;fill:none;stroke:currentColor;stroke-linecap:round;stroke-linejoin:round}.chat-room .composer-box #sendBtn svg{stroke-width:2.4}.chat-room .composer-box #askReplyBtn svg{stroke-width:1.8}html[data-appearance="dark"] .chat-room .composer-box #askReplyBtn{background:var(--chat-surface);color:var(--chat-muted);border-color:var(--chat-border)}@media(max-width:540px){.chat-room:not(.chat-add-open-v41) .chat-main{min-height:0}}</style>')}
function enhanceComposerMobileUiV101(){injectComposerMobileUiV101();syncMobileViewportV101();syncComposerHeightV101();const main=$('chatMain'),input=$('chatInput'),composer=document.querySelector('.composer');if(main&&!main.dataset.bottomFollowV101){main.dataset.bottomFollowV101='true';main.addEventListener('scroll',()=>{chatBottomFollowV101=isNearChatBottomV101()},{passive:true})}if(composer&&window.ResizeObserver&&!chatComposerObserverV101){chatComposerObserverV101=new ResizeObserver(syncComposerHeightV101);chatComposerObserverV101.observe(composer)}const viewport=window.visualViewport;if(viewport&&!window.__composerViewportV101){window.__composerViewportV101=true;viewport.addEventListener('resize',()=>{syncMobileViewportV101();scrollLatestForKeyboardV22()});viewport.addEventListener('scroll',syncMobileViewportV101)}if(input&&!input.dataset.composerMobileV101){input.dataset.composerMobileV101='true';input.addEventListener('focus',()=>{if(isNearChatBottomV101())chatBottomFollowV101=true;scrollLatestForKeyboardV22()});input.addEventListener('input',syncComposerHeightV101)};['sendBtn','askReplyBtn'].forEach(id=>$(id)?.addEventListener('click',()=>{chatBottomFollowV101=true;scrollToLatestV101({force:true})}));}
const openConversationV101Base=openConversation;
openConversation=async function(id){chatBottomFollowV101=true;const result=await openConversationV101Base(id);syncMobileViewportV101();syncComposerHeightV101();scrollToLatestV101({force:true});return result};
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',enhanceComposerMobileUiV101);else enhanceComposerMobileUiV101();

// V102: visual-only composer regression guard; handlers and viewport code stay untouched.
function applyComposerButtonUiV102(){const start=$('startChatBtn'),reply=$('askReplyBtn'),send=$('sendBtn');if(start)start.innerHTML=ICON.plus;if(reply)reply.innerHTML=ICON.fingerprint;if(send)send.innerHTML=ICON.send}
function injectComposerButtonUiV102(){if($('composerButtonUiV102'))return;document.head.insertAdjacentHTML('beforeend','<style id="composerButtonUiV102">.chat-room .composer-box #askReplyBtn,.chat-room .composer-box #sendBtn{box-sizing:border-box!important;width:48px!important;height:48px!important;min-width:48px!important;min-height:48px!important;flex:0 0 48px!important;align-self:center!important;border-radius:50%!important}.chat-room .composer-box #askReplyBtn svg,.chat-room .composer-box #sendBtn svg{width:22px!important;height:22px!important}.chat-room .composer-box #askReplyBtn svg{stroke-width:1.8!important}.chat-room .composer-box #sendBtn svg{stroke-width:2.2!important}</style>')}
