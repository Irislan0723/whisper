...
// V100: keep the Chat header's two actions focused on navigation and the main menu.
function navigateChatBackV100(){const referrer=document.referrer;try{if(referrer&&new URL(referrer).origin===location.origin&&history.length>1){history.back();return}}catch(_){}location.href='index.html'}
function ensureConversationSettingsEntryV100(){const nav=document.querySelector('#leftDrawer .nav-grid');if(!nav||$('conversationSettingsEntryV100'))return;const button=document.createElement('button');button.type='button';button.className='nav-action';button.id='conversationSettingsEntryV100';button.setAttribute('aria-label','对话设置');button.innerHTML=ICON.settings+'对话设置';button.onclick=()=>openDrawer('right');nav.appendChild(button)}
...
ensureConversationSettingsEntryV100();
