...
function injectModeToggleStylesV80(){if($('modeToggleStylesV80'))return;document.head.insertAdjacentHTML('beforeend','<style id="modeToggleStylesV80">.right-menu-mode-v80{padding:8px 0 6px;margin-bottom:2px}.right-menu-mode-label-v80{display:flex;align-items:center;gap:10px;margin:0 4px 8px;color:var(--chat-muted);font-size:12px}.right-menu-mode-label-v80 .right-menu-icon{display:flex;align-items:center}.mode-segmented-v80{display:flex;width:100%;border-radius:10px;overflow:hidden;border:1px solid var(--chat-border);background:var(--chat-bg)}.mode-segmented-v80 button{flex:1;padding:8px 0;border:0;background:transparent;color:var(--chat-muted);font:13px var(--font-b);cursor:pointer;transition:background .15s,color .15s}.mode-segmented-v80 button.active{background:var(--chat-accent);color:var(--accent-contrast)}.right-menu-agent-info-v80{margin:6px 4px 0;color:var(--chat-muted);font:11px/1.5 var(--font-b)}</style>')}
function ensureModeToggleV80(){const root=$('rightSettingsMenuV12');if(!root||$('modeToggleV80'))return;const modelSection=root.querySelector('.right-menu-model');if(!modelSection)return;const section=document.createElement('section');section.className='right-menu-mode-v80';section.id='modeToggleV80';section.innerHTML='<div class="right-menu-mode-label-v80"><span class="right-menu-icon">'+MODE_ICON_V80+'</span><span>对话模式</span></div><div class="mode-segmented-v80" id="modeSegmentedV80"><button type="button" data-mode="api">API</button><button type="button" data-mode="agent">Agent</button></div><div class="right-menu-agent-info-v80" id="modeAgentInfoV80" hidden>当前 Agent：Claude Code</div>';root.insertBefore(section,modelSection);section.querySelectorAll('[data-mode]').forEach(btn=>{btn.onclick=()=>switchModeV80(btn.dataset.mode)})}
...
function hydrateModeToggleV80(){ensureModeToggleV80();const mode=(current?.mode||'api');const seg=$('modeSegmentedV80');if(seg)seg.querySelectorAll('button').forEach(btn=>btn.classList.toggle('active',btn.dataset.mode===mode));const info=$('modeAgentInfoV80');if(info)info.hidden=(mode!=='agent');setModelPickerLabelV12()}
