...
function initIcons(){$('openLeft').innerHTML=ICON.back;$('openRight').innerHTML=ICON.menu;$('addImage').innerHTML=ICON.image;$('sendBtn').innerHTML=ICON.send;$('askReplyBtn').innerHTML=ICON.fingerprint;$('removeImage').innerHTML=ICON.close;$('closeWorkspace').innerHTML=ICON.back;$('closeRolePicker').innerHTML=ICON.close;$('landingMark').innerHTML='<img src="/icons/claude-logo.png" alt="Claude">';$('startChatBtn').innerHTML=ICON.plus+' New chat';const nav=[['models',ICON.models,'模型设置'],['roles',ICON.role,'角色卡'],['archive',ICON.archive,'归档']];nav.forEach(([id,icon,label])=>{const button=document.querySelector(`[data-panel="${id}"]`);if(button)button.innerHTML=icon+label})}
