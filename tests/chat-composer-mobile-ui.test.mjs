...
test('4. AI reply button uses the local Claude logo instead of an arrow', () => assert.match(app, /\$\('askReplyBtn'\)\.innerHTML=AI_REPLY_CLAUDE_LOGO/));
test('5. AI reply button logo has fixed non-layout sizing', () => assert.match(app, /AI_REPLY_CLAUDE_LOGO='<img class="ai-reply-logo" src="\/icons\/claude-logo\.png" width="22" height="22"/));
