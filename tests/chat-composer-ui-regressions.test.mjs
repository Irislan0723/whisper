...
test('4. reply button final visual guard does not use an arrow', () => assert.match(app, /if\(reply\)reply\.innerHTML=ICON\.fingerprint/));
test('5. reply button receives the inline fingerprint SVG', () => assert.match(app, /ICON\.fingerprint='<svg[^>]*><path d="M5 9\.5a7 7 0 0 1 14 0"/));
...
test('13. dark mode keeps the reply button theme-safe', () => assert.match(app, /html\[data-appearance="dark"\] \.chat-room \.composer-box #askReplyBtn/));
